...
router.post('/', async (req, res) => {
    try {
        const { domain, transaction_id, order_id, update_target, return_request, bpp_id, bpp_uri } = req.body;
        const context = build({ domain, action: 'update', transactionId: transaction_id });
        const message = { order: { id: order_id, update_target, return_request } };
