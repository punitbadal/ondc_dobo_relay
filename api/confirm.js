...
router.post('/', async (req, res) => {
    try {
        const { domain, transaction_id, bpp_id, bpp_uri, order, payment } = req.body;

        const context = build({ domain, action: 'confirm', transactionId: transaction_id });

        const message = { order: { ...order, payment } };
