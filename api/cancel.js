...
router.post('/', async (req, res) => {
    try {
        const { domain, transaction_id, order_id, reason_id, bpp_id, bpp_uri } = req.body;
        const context = build({ domain, action: 'cancel', transactionId: transaction_id });
        const message = { order: { id: order_id, cancellation_reason_id: reason_id } };
